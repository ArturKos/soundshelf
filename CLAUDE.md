...
> D9 complete: LyricsController — injectable CacheLookupFn/FetchFn/CacheStoreFn seams + generation-counter stale-reply guard; MainWindow rewired; 7 test cases (cache hit, empty metadata, fetch+store, stale guard, error, decide() truth table, invalid id). ✅ verified: Linux all 38 tests pass.
> D10 complete: OscilloscopePlugin (WantPcm, `buildPolyline` pure static helper, phosphor-green polyline + idle flat line) + WaveformOverviewPlugin (WantSpectrum, injectable DecodeFn seam, epoch-guarded QtConcurrent decode, `computeEnvelope`/`xToMs`/`msToX` pure static helpers, `handleClick`→`engine->seekMs`); `PlayerEngine::visualizationPcm()` accessor; SpectrumWidget passes live PCM to WantPcm plugins + routes mouse click/drag to WaveformOverviewPlugin. ✅ verified: Linux all 39 tests pass.
...
| Metadata resolver (D7) | **działa** — `core::MetadataResolver`: `hasMissingTags`/`parseFromFilename`/`fillFromParsed` (pure static helpers), `candidateIds`/`syncMissing` (DB + file write-back); filename patterns: `NN-Artist-Title`, `NN-Title`, `Artist-Title`, `NN.Title`, `NN Title`; folder→album; network path via AcoustID+Chromaprint (gated on `acoustid.api_key`); CLI `tags sync [--all|--missing|<id>]`; logging `soundshelf.core.metadata` (z testem) |
...
| LyricsController (D9) | **działa** — `core::LyricsController`: injectable `CacheLookupFn`/`FetchFn`/`CacheStoreFn` seams; generation-counter stale-reply guard; `lyricsCleared()` on every track change; `lyricsReady(plain,synced)` on hit/fetch; static `decide()` helper; MainWindow lambda removed + rewired via controller; logging `soundshelf.core.lyrics` (z testem: cache hit, empty metadata, fetch+store, stale guard, error, decide truth table, invalid id) |
