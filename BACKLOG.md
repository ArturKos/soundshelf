...
3. Tick it `✅` when fully satisfied (feature + Doxygen + unit tests that build &
   ...
   - **EXCEPTION — pure GUI / audio-visual behaviour** (on-screen rendering,
     audible playback, live visualisation, real-time widget interaction): the
     loop CANNOT verify these (the tester never launches the GUI or hears audio).
     For such items, once the code + unit-tested seams/logic are done, mark them
     **`🔬`** ("code-complete, awaiting human GUI/audio confirmation"), NOT `✅`.
     Do not claim a visual/audible behaviour works — only that the seam is wired
     and unit-tested.
4. Pick the **next `⬜`** item as the iteration's task. `🔬` items are NOT re-picked.
5. When **every item below is `✅` or `🔬`**, emit `all_implemented` and STOP.
   (`🔬` items are code-complete; the human still has to confirm them on screen.)

Status legend: `⬜` todo · `✅` done & verified · `🔬` code-complete, needs human GUI/audio check.
...
When sections A, B and D are all `✅` or `🔬` (verified against the code, tests
green on Linux, builds clean on Linux and Windows, CI green; `🔬` = code-complete
but the on-screen/audible behaviour still needs the human's eyes/ears), the
architecture is **complete** — emit `all_implemented`. Everything else lives in
section C and is the human's call.
